...
def plot_user_distribution(rfm_df, save_path='../outputs/user_distribution.png'):
    """
    绘制用户分群分布图
...
def plot_rfm_heatmap(rfm_df, save_path='../outputs/rfm_heatmap.png'):
    """
    绘制 RFM 热力图
    
    展示 Frequency 和 Monetary 在不同 Recency 区间的分布
    """
    # 创建 Recency 分组
    rfm_df = rfm_df.copy()
    rfm_df['R_Group'] = pd.cut(rfm_df['Recency'], 
                                bins=[0, 30, 60, 90, 180, 400],
                                labels=['0-30天', '31-60天', '61-90天', '91-180天', '180天+'])
    
    # 创建透视表
    pivot_table = rfm_df.groupby('R_Group').agg({
        'Frequency': 'mean',
        'Monetary': 'mean'
    })
    ...
    # Frequency 热力图
    sns.heatmap(pivot_table[['Frequency']], annot=True, fmt='.1f', 
                cmap='YlOrRd', ax=axes[0], cbar_kws={'label': '平均购买频次'})
    axes[0].set_title('各活跃区间的平均购买频次', fontsize=14)
    axes[0].set_xlabel('')
    axes[0].set_ylabel('最近购买时间', fontsize=12)
    
    # Monetary 热力图
    sns.heatmap(pivot_table[['Monetary']], annot=True, fmt='.0f',
...
def plot_monetary_distribution(rfm_df, save_path='../outputs/monetary_distribution.png'):
    """绘制各群体的消费金额分布箱型图"""
...
def plot_frequency_distribution(rfm_df, save_path='../outputs/frequency_distribution.png'):
    """绘制各群体的购买频次分布"""
...
def plot_clustering_scatter(rfm_df, save_path='../outputs/user_clustering.png'):
    """
    绘制用户聚类散点图
    
    需要 rfm_df 包含 Cluster 列
    """
...
def plot_segment_comparison(rfm_df, save_path='../outputs/segment_comparison.png'):
    """绘制各群体的 RFM 均值对比雷达图"""
    from math import pi
...
def generate_all_plots(rfm_df, output_dir='../outputs'):
    ...
    # 2. RFM 热力图
    plot_rfm_heatmap(rfm_df, f'{output_dir}/rfm_heatmap.png')
    ...
    # 5. 聚类散点图
    plot_clustering_scatter(rfm_df, f'{output_dir}/user_clustering.png')
    
    # 6. 雷达图对比
    plot_segment_comparison(rfm_df, f'{output_dir}/segment_comparison.png')
...
if __name__ == '__main__':
    # 测试代码
    import pandas as pd
    rfm_df = pd.read_csv('../outputs/rfm_results.csv', index_col=0)
